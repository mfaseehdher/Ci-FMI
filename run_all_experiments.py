"""
run_all_experiments.py -- Generic experiment runner for FMU co-simulation

Discovers and runs all experiments automatically.

Folder structure expected:
  experiments/
    <category>/
      <model_name>/
        <model_name>.json       experiment config
        <model_name>_ref.csv    MATLAB reference CSV
        <model_name>.fmu        exported FMU

Usage:
  python run_all_experiments.py
  python run_all_experiments.py --experiments experiments
  python run_all_experiments.py --tol 0.01
  python run_all_experiments.py --category first_order
  python run_all_experiments.py --model cruise_control
  python run_all_experiments.py --no-plots

Exit codes:
  0  all experiments passed
  1  one or more experiments failed
  2  no experiments found
"""
import argparse
import csv
import json
import os
import subprocess
import sys
from datetime import datetime
from typing import List, Optional


REPO_ROOT = os.path.dirname(os.path.abspath(__file__))


def discover_experiments(
    experiments_dir: str,
    model_filter: Optional[str] = None,
    category_filter: Optional[str] = None,
) -> List[dict]:
    experiments = []

    if not os.path.isdir(experiments_dir):
        print(f"ERROR: experiments directory not found: {experiments_dir}")
        return []

    def try_add(model_dir, model_name, category):
        if not os.path.isdir(model_dir):
            return

        if category_filter and category != category_filter:
            return

        json_files = [f for f in os.listdir(model_dir) if f.endswith(".json")]
        if not json_files:
            return

        json_file = next(
            (jf for jf in json_files if os.path.splitext(jf)[0] == model_name),
            json_files[0],
        )

        json_path = os.path.join(model_dir, json_file)
        case_name = os.path.splitext(json_file)[0]

        if model_filter and model_filter not in (model_name, case_name):
            return

        ref_csv = os.path.join(model_dir, f"{case_name}_ref.csv")
        if not os.path.isfile(ref_csv):
            refs = sorted(f for f in os.listdir(model_dir) if f.endswith("_ref.csv"))
            ref_csv = os.path.join(model_dir, refs[0]) if refs else None

        has_ref = ref_csv is not None and os.path.isfile(ref_csv)

        display = f"{category}/{model_name}"
        experiments.append({
            "name": case_name,
            "display_name": display,
            "category": category,
            "dir": model_dir,
            "json": json_path,
            "ref_csv": ref_csv if has_ref else None,
        })

        status = "with reference" if has_ref else "no reference"
        print(f"[discover] Found: {display} ({status})")

    for category in sorted(os.listdir(experiments_dir)):
        cat_path = os.path.join(experiments_dir, category)
        if not os.path.isdir(cat_path):
            continue

        for model_name in sorted(os.listdir(cat_path)):
            model_dir = os.path.join(cat_path, model_name)
            try_add(model_dir, model_name, category)

    return experiments


def run_experiment(exp: dict, tol: float, generate_plots: bool, plots_dir: str) -> dict:
    name = exp["name"]
    model_dir = exp["dir"]
    json_path = exp["json"]
    ref_csv = exp["ref_csv"]

    result = {
        "name": name,
        "display_name": exp["display_name"],
        "category": exp["category"],
        "passed": False,
        "errors": [],
        "skipped": False,
        "json": json_path,
        "ref_csv": ref_csv,
        "results_csv": None,
        "metrics_csv": None,
    }

    print(f"\n{'=' * 60}")
    print(f"  Running: {exp['display_name']}")
    print(f"{'=' * 60}")

    try:
        with open(json_path) as f:
            cfg = json.load(f)

        results_csv = None
        for comp_spec in cfg.get("components", {}).values():
            if comp_spec.get("type") == "logger":
                fname = comp_spec.get("file", f"results_{name}.csv")
                results_csv = (
                    os.path.join(model_dir, fname)
                    if not os.path.isabs(fname)
                    else fname
                )
                break

        if not results_csv:
            results_csv = os.path.join(model_dir, f"results_{name}.csv")

        result["results_csv"] = results_csv

    except Exception as e:
        result["errors"].append(f"Cannot read JSON: {e}")
        return result

    print(f"\n[{name}] Step 1/3: Simulating...")
    generic_py = os.path.join(REPO_ROOT, "generic.py")

    sim = subprocess.run(
        [sys.executable, generic_py, os.path.abspath(json_path)],
        cwd=model_dir,
        capture_output=True,
        text=True,
    )

    print(sim.stdout)
    if sim.returncode != 0:
        result["errors"].append(f"Simulation failed:\n{sim.stderr}")
        print(f"[{name}] SIMULATION FAILED")
        print(sim.stderr)
        return result

    print(f"[{name}] Simulation OK")

    if ref_csv is None:
        print(f"[{name}] Step 2/3: No reference CSV -- skipping validation")
        result["passed"] = True
        result["skipped"] = True
    else:
        print(f"[{name}] Step 2/3: Comparing against reference...")
        compare_py = os.path.join(REPO_ROOT, "compare.py")
        metrics_csv = os.path.join(model_dir, f"metrics_{name}.csv")
        result["metrics_csv"] = metrics_csv

        cmp = subprocess.run(
            [
                sys.executable,
                compare_py,
                os.path.abspath(ref_csv),
                os.path.abspath(results_csv),
                "--tol",
                str(tol),
                "--metrics-csv",
                os.path.abspath(metrics_csv),
                "--no-append",
            ],
            capture_output=True,
            text=True,
        )

        print(cmp.stdout)
        if cmp.returncode == 0:
            result["passed"] = True
            print(f"[{name}] PASS")
        else:
            result["errors"].append("RMSE exceeded tolerance")
            print(f"[{name}] FAIL")
            print(cmp.stderr)

    if generate_plots and ref_csv is not None:
        print(f"[{name}] Step 3/3: Generating plots...")
        model_plots = os.path.abspath(os.path.join(plots_dir, exp["category"], name))
        os.makedirs(model_plots, exist_ok=True)

        plot_py = os.path.join(REPO_ROOT, "plot_results.py")
        plot_cmd = [
            sys.executable,
            plot_py,
            os.path.abspath(ref_csv),
            os.path.abspath(results_csv),
            "--title",
            name,
            "--output",
            model_plots,
        ]

        if os.path.isfile(os.path.join(model_dir, "coupled_experiment.json")):
            plot_cmd.extend([
                "--reference-label",
                "MATLAB closed-loop reference",
                "--output-label",
                "Python coupled FMU co-simulation",
                "--comparison-title",
                "Closed-loop reference vs coupled FMU co-simulation",
            ])

        plot = subprocess.run(
            plot_cmd,
            capture_output=True,
            text=True,
        )

        if plot.returncode == 0:
            print(f"[{name}] Plots saved to {model_plots}")
        else:
            print(f"[{name}] Plot generation failed")
            print(plot.stderr)

    elif generate_plots:
        print(f"[{name}] Step 3/3: No reference -- skipping plots")

    return result


def write_validation_summary(results: List[dict], tol: float, summary_csv: str) -> None:
    """Collect per-experiment metric CSV files into one thesis-ready report."""
    fieldnames = [
        "run_at",
        "tolerance",
        "category",
        "experiment",
        "display_name",
        "signal",
        "RMSE",
        "MAE",
        "Max_abs_err",
        "Rel_RMSE_percent",
        "DTW",
        "Status",
        "run_status",
        "reference_csv",
        "results_csv",
        "metrics_csv",
        "json_file",
        "notes",
    ]

    run_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    rows = []

    for r in results:
        if r["skipped"]:
            run_status = "RUN-ONLY"
        elif r["passed"]:
            run_status = "PASS"
        else:
            run_status = "FAIL"

        base = {
            "run_at": run_at,
            "tolerance": tol,
            "category": r.get("category", ""),
            "experiment": r.get("name", ""),
            "display_name": r.get("display_name", ""),
            "run_status": run_status,
            "reference_csv": r.get("ref_csv") or "",
            "results_csv": r.get("results_csv") or "",
            "metrics_csv": r.get("metrics_csv") or "",
            "json_file": r.get("json") or "",
            "notes": " | ".join(r.get("errors", [])),
        }

        metrics_csv = r.get("metrics_csv")
        if metrics_csv and os.path.isfile(metrics_csv):
            with open(metrics_csv, newline="") as f:
                reader = csv.DictReader(f)
                for metric_row in reader:
                    row = dict(base)
                    row.update({
                        "signal": metric_row.get("signal", ""),
                        "RMSE": metric_row.get("RMSE", ""),
                        "MAE": metric_row.get("MAE", ""),
                        "Max_abs_err": metric_row.get("Max_abs_err", ""),
                        "Rel_RMSE_percent": metric_row.get("Rel_RMSE_percent", ""),
                        "DTW": metric_row.get("DTW", ""),
                        "Status": metric_row.get("Status", run_status),
                    })
                    rows.append(row)
        else:
            row = dict(base)
            row.update({
                "signal": "",
                "RMSE": "",
                "MAE": "",
                "Max_abs_err": "",
                "Rel_RMSE_percent": "",
                "DTW": "",
                "Status": run_status,
            })
            rows.append(row)

    with open(summary_csv, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)

    print(f"[runner] Validation summary written to: {summary_csv}")


def print_summary(results: List[dict], tol: float) -> bool:
    print(f"\n{'=' * 65}")
    print("  VALIDATION SUMMARY")
    print(f"  Tolerance : {tol}")
    print(f"  Run at    : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'=' * 65}")

    categories = {}
    for r in results:
        cat = r.get("category", "uncategorized")
        categories.setdefault(cat, []).append(r)

    col = max(len(r["display_name"]) for r in results) + 2
    all_passed = True

    for cat_name, cat_results in sorted(categories.items()):
        print(f"\n  [{cat_name.upper()}]")
        print(f"  {'Model':<{col}}  {'Status':>8}")
        print(f"  {'-' * col}  {'--------'}")

        for r in cat_results:
            if r["skipped"]:
                status = "RUN-ONLY"
            elif r["passed"]:
                status = "PASS"
            else:
                status = "FAIL"
                all_passed = False

            notes = " | ".join(r["errors"]) if r["errors"] else ""
            print(f"  {r['display_name']:<{col}}  {status:>8}  {notes}")

    total = len(results)
    passed = sum(1 for r in results if r["passed"])
    failed = sum(1 for r in results if not r["passed"] and not r["skipped"])
    skipped = sum(1 for r in results if r["skipped"])

    print(f"\n{'=' * 65}")
    print(f"  Total: {total}  Passed: {passed}  Failed: {failed}  Run-only: {skipped}")
    print(f"  Overall verdict: {'ALL PASSED' if all_passed else 'SOME FAILED'}")
    print(f"{'=' * 65}\n")

    return all_passed


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run all FMU co-simulation experiments."
    )
    parser.add_argument("--experiments", default="experiments")
    parser.add_argument("--tol", type=float, default=0.01)
    parser.add_argument("--model", default=None)
    parser.add_argument("--category", default=None)
    parser.add_argument("--no-plots", action="store_true")
    parser.add_argument("--plots-dir", default="plots")
    parser.add_argument("--summary-csv", default="validation_summary.csv")
    args = parser.parse_args()

    print(f"\n[runner] Discovering experiments in: {args.experiments}")

    experiments = discover_experiments(
        args.experiments,
        model_filter=args.model,
        category_filter=args.category,
    )

    if not experiments:
        print("ERROR: no experiments found.")
        return 2

    print(f"[runner] Found {len(experiments)} experiment(s)")
    print(f"[runner] Tolerance: {args.tol}")

    results = []
    for exp in experiments:
        result = run_experiment(
            exp,
            tol=args.tol,
            generate_plots=not args.no_plots,
            plots_dir=args.plots_dir,
        )
        results.append(result)

    write_validation_summary(results, args.tol, args.summary_csv)
    all_passed = print_summary(results, args.tol)
    return 0 if all_passed else 1


if __name__ == "__main__":
    sys.exit(main())
