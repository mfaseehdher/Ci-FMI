% setup_stiff_rlc_circuit_closed_loop.m
% Coupled two-FMU validation case for a stiff RLC circuit plant.

testDir = pwd;
caseName = 'stiff_rlc_circuit_closed_loop';
t_stop = 2;
dt = 0.0005;

plantModel = 'Stiff_RLC_Plant';
controllerModel = 'Stiff_RLC_Controller';
referenceModel = 'Stiff_RLC_ClosedLoop_Reference';
...
create_stiff_rlc_plant(plantModel, L, C, R, t_stop, dt);
export_model(plantModel, testDir);

create_p_controller(controllerModel, Kp, t_stop, dt);
export_model(controllerModel, testDir);

create_stiff_rlc_reference(referenceModel, L, C, R, Kp, t_stop, dt);
run_reference(referenceModel, caseName, 'CapacitorVoltage');

write_coupled_json(testDir, caseName, t_stop, dt, ...
    'Stiff_RLC_Controller.fmu', 'Stiff_RLC_Plant.fmu', ...
    'ReferenceVoltage', 'MeasuredVoltage', 'SourceVoltage', 'CapacitorVoltage');


function create_stiff_rlc_plant(model, L, C, R, t_stop, dt)
reset_model(model);
new_system(model);
add_block('simulink/Sources/In1', [model '/SourceVoltage'], 'Position', [40 80 70 100]);
add_block('simulink/Continuous/Transfer Fcn', [model '/Stiff_RLC_Circuit'], ...
    'Numerator', '1', 'Denominator', sprintf('[%g %g 1]', L * C, R * C), ...
    'Position', [150 70 300 110]);
add_block('simulink/Sinks/Out1', [model '/CapacitorVoltage'], 'Position', [380 80 410 100]);
add_line(model, 'SourceVoltage/1', 'Stiff_RLC_Circuit/1', 'autorouting', 'on');
add_line(model, 'Stiff_RLC_Circuit/1', 'CapacitorVoltage/1', 'autorouting', 'on');
...
function create_stiff_rlc_reference(model, L, C, R, Kp, t_stop, dt)
reset_model(model);
new_system(model);
...
add_block('simulink/Continuous/Transfer Fcn', [model '/Stiff_RLC_Circuit'], ...
    'Numerator', '1', 'Denominator', sprintf('[%g %g 1]', L * C, R * C), ...
    'Position', [340 80 490 120]);
add_block('simulink/Sinks/Out1', [model '/CapacitorVoltage'], 'Position', [570 90 600 110]);
add_line(model, 'ReferenceVoltageStep/1', 'Error/1', 'autorouting', 'on');
add_line(model, 'FeedbackDelay/1', 'Error/2', 'autorouting', 'on');
add_line(model, 'Error/1', 'Kp/1', 'autorouting', 'on');
add_line(model, 'Kp/1', 'Stiff_RLC_Circuit/1', 'autorouting', 'on');
add_line(model, 'Stiff_RLC_Circuit/1', 'CapacitorVoltage/1', 'autorouting', 'on');
add_line(model, 'Stiff_RLC_Circuit/1', 'FeedbackDelay/1', 'autorouting', 'on');
configure_model(model, t_stop, dt);
save_system(model);
end
